...
void Floor_TextureRender(Camera_Struct* camera) {
    ...
    for (int scrRow = SCREEN_ROW / 2 + 1; scrRow < SCREEN_ROW; scrRow++) {
        ...
        for (int scrCol = 0; scrCol < SCREEN_COLUMN; scrCol++) {
            ...
            int indexFloor = mapIdxRow * camera->Floor.Col + mapIdxCol;
            unsigned char tileIndexFloor = camera->Floor.floorDescriptArr[indexFloor];
            ...
            int texRows = floorTexNode->ROW;
            int texCols = floorTexNode->COL;
            int texRow = (int)(fracRow * texRows) % texRows;
            int texCol = (int)(fracCol * texCols) % texCols;
            unsigned char floorPixelVal = floorTexNode->FC_texture[ texRow * texCols + texCol ];
            CANVAS_PIXEL_STATUS floorPixelStatus = (floorPixelVal > 0) ? CANVAS_PIXEL_OPEN : CANVAS_PIXEL_CLOSE;

            Canvas_PixelOperate(scrRow, scrCol, floorPixelStatus);
