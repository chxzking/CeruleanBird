	...
	if(Sprites->BehaviorMoudle.IT_Enable == 1){//执行中断表
		...
		switch(result){
			case 0:
				return 0;
			case 1:
				goto SAT_EXEC_FLAG;//跳转非完备帧验证持续执行
			case 2:
				goto BT_EXEC_FLAG;//跳转普通行为树执行
			default:
				return -1;
		}
	}
	
BT_EXEC_FLAG:;/*行为树执行标签 (@note 历史遗留问题，原计划区分完备与非完备标签，分类实现，但是根据测试发现存在问题，
				会导致非完备标签被跳过)*/
SAT_EXEC_FLAG:;	//非完备帧连续执行确认节点
	if(Sprites->BehaviorMoudle.bt_context.SBB_NodeHandleType != SBB_NO){//如果上次执行的节点不为空节点
		SpritesPhased_Exec(Sprites);
		return 0;
	}
//BT_EXEC_FLAG:;//行为树执行标签 	
	//执行行为树
...
*		@arg	激活失败返回-1
*/
int SpritesIT_Trigger(Sprites_t* Sprites){
	if(Sprites == NULL || Sprites->BehaviorMoudle.IT_Enable == 0){
		return -1;
	}
	Sprites->BehaviorMoudle.IT_flag = 1;
	return 0;
}
/**
*	@brief	清楚指定精灵的行为树的中断
